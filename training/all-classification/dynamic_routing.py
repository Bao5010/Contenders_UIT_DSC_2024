...
from dynrt_model import create_dynrt_model
...
def train_model(model, train_loader, num_epochs=3, device='cuda'):
    ...
    for epoch in range(num_epochs):
        ...
        if train_loss < best_loss:
            ...
            torch.save(model.state_dict(), 'best_dynrt_model.pth')
...
def main():
    ...
    train_dataset = SarcasmDataset(
        json_file='training/vimmsd-train.json',
    ...
    model.load_state_dict(torch.load('best_dynrt_model_multi_not.pth'))
    model.to(device)
    ...
    with open('results_multi_not.json', 'w', encoding='utf-8') as f:
