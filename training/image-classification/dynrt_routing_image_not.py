...
def train_model(model, train_loader, num_epochs=3, device='cuda'):
    ...
    for epoch in range(num_epochs):
        ...
        if epochs_no_improve >= early_stop_patience:
            torch.save(model.state_dict(), 'dynrt_model_image_not.pth')
...
def main():
    ...
    model.load_state_dict(torch.load('dynrt_model_image_not.pth'))
    model.to(device)
    ...
    with open('statistics/results_image_not.json', 'w', encoding='utf-8') as f:
