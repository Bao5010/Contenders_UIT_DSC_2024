...
def preprocess_text(text):
    ...
    # Normalize words (remove extra spaces)
...
class MultiModalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        caption_text = preprocess_text(item['caption'])
        caption = self.tokenizer(caption_text, return_tensors='pt', padding='max_length', truncation=True, max_length=256)
...
image_model = AutoModel.from_pretrained('google/vit-base-patch16-384')
...
# Save the model
torch.save(model.state_dict(), 'model-fusion-not-bridge-layer.pth')