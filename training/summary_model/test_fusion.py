import json
import torch
from transformers import AutoTokenizer, AutoImageProcessor, AutoModelForSequenceClassification, ViTModel, AutoModel
...
image_multi_model = ViTModel.from_pretrained('google/vit-base-patch16-384', image_size = 384)
text_multi_model = AutoModel.from_pretrained('vinai/phobert-base-v2')

# Initialize the multi-modal model
multi_model = MultiModalModel(image_multi_model, text_multi_model, num_labels=2)
multi_model.load_state_dict(torch.load('./model-fusion-not-bridge-layer.pth'))

# Load pre-trained models
image_model = ViTModel.from_pretrained('google/vit-base-patch16-384', image_size = 384)
...
with open('summary-results.json', 'w', encoding='utf-8') as f:
