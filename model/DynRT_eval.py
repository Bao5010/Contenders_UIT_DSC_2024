...
class SarcasmDataset(Dataset):
    def __init__(self, json_file, img_dir, tokenizer, transform=None, max_length=256, is_test=False):
...
def main():
    ...
    train_dataset = SarcasmDataset(
        'training/multi-classification/split_multi_not_sarcasm.json',
        'train-images/',
        tokenizer,
    ...
    eval_dataset = SarcasmDataset(
        'testing/vimmsd-test-split.json',
        'train-images/',
        tokenizer,
        max_length=config['opt']['modelopt']['len']
    )
    ...
    for epoch in range(config['opt']['total_epoch']):
        ...
        print(f"Evaluation_loss: {eval_loss:.4f}, Evaluation Acc: {eval_acc:.2f}%")
    ...
    with open('statistics/' + 'best_model_multi.json', 'w') as f:
